...
export class LinegraphComponent implements OnInit, OnChanges {
  ...
  usersChartRender() {
    ...
    const xScale = d3
      .scaleTime()
      .domain(
        d3.extent(this.lineGraphData, function (d) {
          return d[0];
        })
      )
      .range([p, w - p / 2]);
    ...
    // create yScale
    const yMax = d3.max(this.lineGraphData, function (d) {
    ...
    svg
      .append('path')
      .datum(this.lineGraphData) // Binds data to the line
  ...
  ngOnChanges(changes: any) {
    if (changes.lineGraphData && changes.lineGraphData.currentValue) {
