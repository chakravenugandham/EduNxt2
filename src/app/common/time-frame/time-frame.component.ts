...
export class TimeFrameComponent implements OnInit {
    ...
    // Whenever the date has been changed or the tanent has been changed this methode will be called to reset the dropdown value  
    changeDropDown() {
        this.programsData = [];
        this.selectProgram = "All Programs";
        this.selectCourse = "All Courses";
        this.selectBatch = "All Batches";
        this.selectSection = "All Sections";
        this.programObj.programId = 0;
        this.programObj.courseId = 0;
        this.programObj.batchId = 0;
    ...
    // To get the initial dropdown for peogram dropdown
    getPrograms() {
        this.dashboardService.getProgramData().subscribe((res: any) => {
            this.programsData = res.data;
        });
    }
    // Up on selecting the program the course drop down will get reflected
    getCourses(programId) {
        this.dashboardService.getCoursesData(programId).subscribe((res: any) => {
            this.coursesData = res.data;
        });
    }
    // Up on selecting the course and program the batch drop down will get reflected
    ...
    // Up on selecting the course, program and batch the sections drop down will get reflected
    ...
    // Calls when the apply button clicked to reflect the data in dashboard according to filters selected
    applyPrograms() {
        //this.dashboardService.courseAndProgram(this.programObj);
        //this.refreshAPI$.next();
        this.dashboardService.applyDropdownButton(this.programObj);
    }
