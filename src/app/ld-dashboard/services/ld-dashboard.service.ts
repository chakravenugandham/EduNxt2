...
export class LdDashboardService implements OnInit {
  ...
  getPerformanceDetails(searchFilterData, searchTerm, pagination) {
    const url = this.baseURL + searchFilterData.searchComponent + this.setDateObj + this.program_course +
      '&searchBy=' + searchFilterData.searchBy + '&searchTerm=' + searchTerm + '&page=' + pagination.page + '&limit=' + pagination.limitTo;
    return this.http.post(url, null, { headers: this.headers });
  }
